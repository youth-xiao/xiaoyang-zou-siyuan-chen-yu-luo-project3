...
const checkTweetOwnership = async function (request, response, next) {
    ...
    try {
        ...
        if (foundTweet.username !== username) {
            response.status(403);
            return response.send("You do not have permission to perform this action.");
        }
 ...
 */
router.delete("/id/:tweetId", checkTweetOwnership, async function (request, response) {
    const tweetId = request.params.tweetId;
    if (!tweetId) {
        response.status(400);
        return response.send("Tweet ID parameter is missing.");
 ...
 */
router.put("/id/:tweetId", checkTweetOwnership, async function (request, response) {
    const tweetId = request.params.tweetId;
    ...
    try {
        const updatedTweet = await TweetAccessor.updateTweetById(tweetId, updatedContent);
        if (!updatedTweet) {
            response.status(404);
            return response.send("Tweet not found.");
        }
